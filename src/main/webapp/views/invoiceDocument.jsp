...
<div class="d-grid gap-2 d-md-flex justify-content-md-end mb-2">
    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target=".bd-example-modal-c" type="button"><i
            class="fa-solid fa-plus mx-sm-1"></i>Yuk xati qo'shish
    </button>
</div>
        ...
        <div class="card-body">
            <table class="table table-hover my-0 dataTable">
                ...
                <tr>
                    ...
                    <td><%=invoiceDocumentDTO.getDocumentNumber()%>
                    </td>
                    <td><%=invoiceDocumentDTO.getOrganization().getName()%>
                    </td>
                    <td><%=invoiceDocumentDTO.getTotalSumma()%>
                    </td>
                    <td><%=invoiceDocumentDTO.getDate()%>
                    </td>
                    <td>
                        <a href="<%=request.getContextPath()%>/market/view_invoice_document_item/<%=invoiceDocumentDTO.getId()%>"
                           class="btn btn-primary btn-sm"><i class="fa-solid fa-eye mx-sm-1"></i>Ko'rish</a>
                        <button type="button" data-bs-toggle="modal"
                                data-bs-target=".bd-example-modal-a<%=invoiceDocumentDTO.getId()%>"
                                class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square mx-sm-1"></i>O'zgartirish
                        </button>
                        <button type="button" data-bs-toggle="modal"
                                data-bs-target=".bd-example-modal-b<%=invoiceDocumentDTO.getId()%>"
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <a href="<%=request.getContextPath()%>/market/delete_invoice_document/<%=invoiceDocumentDTO.getId()%>"
                    ...
                    <%--Modal for update--%>
                    <div class="modal fade bd-example-modal-a<%=invoiceDocumentDTO.getId()%>"
                         aria-labelledby="exampleModalEditCategory" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title" id="exampleModalEditCategory"><b>Yuk xati o'zgartirish</b>
                                    </h4>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                </div>
                                <form action="update_invoice_document" method="post">
                                    <input type="hidden" name="id" class="form-control exampleDocId"
                                           value="<%=invoiceDocumentDTO.getId()%>">
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <label for="exampleDocNumber" class="form-label">Yuk xati raqami</label>
                                            <input type="text" name="docNum" class="form-control"
                                                   id="exampleDocNumber"
                                                   value="<%=invoiceDocumentDTO.getDocumentNumber()%>" required>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="organizationId" id="exampleOrganization2"
                                                    class="form-control exampleOrganization"
                                                    required>
                                                <option value="" selected>Tashkilotni tanlang</option>
                                                <% for (OrganizationDTO organizationDTO : organizationList) {%>
                                                <option value="<%=organizationDTO.getId()%>" <%=organizationDTO.getId() == invoiceDocumentDTO.getOrganization().getId() ? "selected" : "" %>><%=organizationDTO.getName()%>
                                                </option>
                                                <%}%>
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            ...
                                            <input type="date" name="date" class="form-control exampleDate2"
                                                   id="exampleDate" value="<%=invoiceDocumentDTO.getDate()%>" required>
                                        </div>
                                        <div class="modal-footer">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="create_invoice_document" method="post">
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="exampleDocNumber5" class="form-label">Yuk xati raqami</label>
                        <input type="text" name="docNum" class="form-control exampleDocNumber2"
                               id="exampleDocNumber5"
                               onchange="checkInvoiceDocumentNumberAJAX(document.getElementById('exampleOrganization').value,this.value,document.getElementById('exampleDate2').value)"
                               required>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="organizationId" id="exampleOrganization" class="form-control exampleOrganization"
                                required>
                            <option value="" selected>Tashkilotni tanlang</option>
                            <% for (OrganizationDTO organizationDTO : organizationList) {%>
                            <option value="<%=organizationDTO.getId()%>"><%=organizationDTO.getName()%>
                            </option>
                            <%}%>
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="date" name="date" class="form-control exampleDate2" id="exampleDate2" required>
                    </div>
                    <div class="modal-footer">
...
<script>
    $(document).ready(function () {
        $(".exampleOrganization, .exampleDocNumber2, .exampleDate2, .exampleDocId").on("change", function () {
        ...
        if (organizationId && documentNumber && date) {
            $.ajax({
                ...
                success: function (data) {
                    if (data === true) {
                        $(".exampleDocNumber2").addClass("is-invalid");
                        $(".saveButton").prop('disabled', true);
                    } else {
                        $(".exampleDocNumber2").removeClass("is-invalid");
